...
if(isset($_POST["view"]))
{
	...
	$sql = "SELECT complaints.client_id,customer.fname,customer.lname,complaints.issue,complaints.issuedesc,property.name,complaints.startdate,complaints.priority,complaints.`status`,complaints.`type`,customer.unum,customer.owner,complaints.image,complaints.solution,complaints.cost,complaints.enddate,a.fname as ifname,a.lname as ilname,complaints.c_id,complaints.incharge,complaints.authorizer from complaints 
							...
							where complaints.incharge = '$e_id'
		...
		<div class="row">
			...
			<div class="col-md-12">
			  	<div class="form-group">
			    	<label for="formGroupExampleInput2">Solution of the issue</label>
			    		<div class="container">
			    		<div class="comment">
			    		<textarea class="textinput" placeholder="Enter the issue description" name="solve"></textarea>
			  			</div>
					</div>
			  		</div>
			</div>
			...
			<div class="col-md-4">
			  <div class="form-group">
			    ...
			    <input type="text" class="form-control"  value = ""id="formGroupExampleInput2" placeholder="Cost Incurred In Kshs." name="cost" required="" >
			  </div>
			</div>
			 	<?php
			}
...
{
  $solve = $conn->escape_string($_POST['solve']);
  $dateofentry =($_POST['dateofentry']);
  $cost = ($_POST['cost']);
  $status = ($_POST['status']);
  $complaintid = ($_POST['complaintid']);
  $updatequery = "UPDATE complaints set solution = '$solve', status = '$status',cost = '$cost',enddate = '$dateofentry' WHERE complaints.c_id = '$complaintid';";
